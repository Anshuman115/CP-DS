...
from 1st column actual subset follows. For eg. Input : {1, 3, 4, 2} and K = 5,
then output array should contain
...
int subsetSumToK(int input[], int n, int startIndex, int output[][50], int k) {
  ...
  int size2 = subsetSumToK(input, n, startIndex + 1, smallOutput2,
                           k - input[startIndex]);
  int row = 0;
